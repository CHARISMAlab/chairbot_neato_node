...
class NeatoNode:
    ...
    def __init__(self):
        ...
        if chairbot_number=='04':
            ...
            rospy.Subscriber("/neato01/pose", PoseStamped, self.pose_handlera, queue_size=10)
            rospy.Subscriber("/neato06/pose", PoseStamped, self.pose_handlerb, queue_size=10)
            rospy.Subscriber("/neato03/pose", PoseStamped, self.pose_handlerc, queue_size=10)

        # Person
        rospy.Subscriber("/neato02/pose", PoseStamped, self.person_handler, queue_size=10)
    ...
    def get_goal_circle(self):
        ang_increment = 2*3.1415/self.num_neatos
        num_neatos = 3
        if self.i_am <=num_neatos:
            neato = self.i_am
        else:
            neato = self.num_neatos

        self.goal_x = self.radius*cos(neato*ang_increment) + self.center_x
        self.goal_y = self.radius*sin(neato*ang_increment) + self.center_y
    ...
    def person_adjust(self):
        ...
        if person_dist01 < person_dist04 or person_dist01 < person_dist03:
            if person_dist04 < person_dist03:
                ...
                #     # closer to 01 so i move to 180
                #     # robot03 (at 240) will move to 270
                #     # robot04 is still
                self.goal_x = self.radius*cos(3.1415) + self.center_x
                self.goal_y = self.radius*sin(3.1415) + self.center_y
                # else:
                #     # closer to 04 so 04 moves to 300
                #     # robot03 (at 240) will move to 210
                #     # robot01 is still
                #     pass
                ...
                #     # closer to 01 so i move to 60
                #     # robot04 (at 0/360) will move to 330
                #     # robot03 is still
                self.goal_x = self.radius*cos(3.1415*60/180) + self.center_x
                self.goal_y = self.radius*sin(3.1415*60/180) + self.center_y
                # else:
                #     # closer to 03 so 03 move to 300
                #     # robot04 (At 0/360) will move to 30
                #     # robot01 is still
                #     pass
        else:
            # between 240 and 360
            # if person_dist04 < person_dist03:
                ...
                # robot03 is still
            self.goal_x = self.radius*cos(3.1415*150/180) + self.center_x
            self.goal_y = self.radius*sin(3.1415*150/180) + self.center_y
            # else:
            #     #closer to robot03 so 04 moves to 180
            #     # robot01 moves to 90
            #     # robot04 still
            #     self.goal_x = self.radius*cos(3.1415*90/180) + self.center_x
            #     self.goal_y = self.radius*sin(3.1415*90/180) + self.center_y
